...
The built-in SoundCloud source is enabled only for a final same-track fallback. The bot sends an explicit `scsearch:` query only after a YouTube URL resolved to trusted non-live metadata and both YouTube playback paths failed. It accepts exactly one encoded, non-live SoundCloud result with exact normalized title/artist/version semantics and duration within two seconds. No OAuth, cookie, PoToken, remote cipher, proxy, or IP-rotation feature is enabled, and SoundCloud is never described as direct YouTube audio.
