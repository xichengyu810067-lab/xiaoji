...
SQLite schema v16 includes disabled-by-default guild feature flags and channel configuration, an idempotent reward-grant ledger tied to `system_reward` coin transactions, a restart-safe delivery outbox, de-identified daily usage counters, a feature health registry, validated text/number chain sessions, generic daily event/message/participant records, and global user chat-style preferences keyed only by the internal Discord user ID. `/word-chain start`, `/number-chain start`, `/daily-riddle enable`, and `/daily-discussion enable` explicitly enable their respective guild feature; chat style is a per-user preference rather than a guild feature.
