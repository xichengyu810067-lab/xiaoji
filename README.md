...
SQLite schema v15 includes disabled-by-default guild feature flags and channel configuration, an idempotent reward-grant ledger tied to `system_reward` coin transactions, a restart-safe delivery outbox, de-identified daily usage counters, a feature health registry, validated text/number chain sessions, and generic daily event/message/participant records. `/word-chain start`, `/number-chain start`, `/daily-riddle enable`, and `/daily-discussion enable` explicitly enable their respective feature; all other planned community features remain disabled and unavailable.
