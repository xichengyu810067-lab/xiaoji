...
function insertOrderItem(api, orderId, menuItem, makerRow, { guildId, date, forceNpc = false } = {}) {
...
function createVenueOrder(
  guildId,
  customerId,
  { mealId = null, drinkId = null, chefId = null, bartenderId = null, channelId = null, date = new Date() } = {}
) {
  return withCoinTransaction((api) => {
    ensureEconomyEnabled(api, guildId);
    ...
    api.run(
      `INSERT INTO casino_venue_orders (guild_id, customer_id, channel_id, status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?)`,
      [guildId, customerId, channelId || null, VenueOrderStatus.PENDING, timestamp, timestamp]
    );
    ...
    if (meal) {
      ...
      items.push(insertOrderItem(api, orderId, meal, maker, { guildId, date, forceNpc: !maker }));
    }
    ...
    if (drink) {
      ...
      items.push(insertOrderItem(api, orderId, drink, maker, { guildId, date, forceNpc: !maker }));
    }
...
function processExpiredVenueOrderItems({ date = new Date() } = {}) {
  return withCoinTransaction((api) => {
    ...
    return { checked: rows.length, completedByNpc: rows.length };
  });
}
